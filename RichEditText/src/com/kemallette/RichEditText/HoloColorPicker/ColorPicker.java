...
public class ColorPicker extends
	 ...
	 * </p>
	 */
	private static final int[]		COLORS					= new int[] {	0xFFFF0000,
						0xFFFF00FF,
						0xFF0000FF,
						0xFF00FFFF,
						0xFF00FF00,
	...
	protected void onDraw(Canvas canvas){
		...
		canvas.translate(	mTranslationOffset,
							mTranslationOffset);

		// Draw the color wheel.
		canvas.drawOval(mColorWheelRectangle,
						mColorWheelPaint);
		...
		canvas.drawArc(	mCenterRectangle,
						90,
						180,
						true,
						mCenterOldPaint);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec){
		...
		if (heightMode == MeasureSpec.EXACTLY)
			...
			height = Math.min(	intrinsicSize,
								heightSize);
		else
			height = intrinsicSize;

		int min = Math.min(	width,
							height);
		setMeasuredDimension(	min,
								min);
		mTranslationOffset = min * 0.5f;

		// fill the rectangle instances.
		mColorWheelRadius = min
							/ 2
		...
		mCenterRectangle.set(	-mColorCenterRadius,
								-mColorCenterRadius,
								mColorCenterRadius,
								mColorCenterRadius);
	}
	...
	private int calculateColor(float angle){
		...
		int c0 = COLORS[i];
		int c1 = COLORS[i + 1];
		int a = ave(Color.alpha(c0),
					Color.alpha(c1),
					p);
		int r = ave(Color.red(c0),
					Color.red(c1),
					p);
		int g = ave(Color.green(c0),
					Color.green(c1),
					p);
		int b = ave(Color.blue(c0),
					Color.blue(c1),
					p);
		...
		switch(event.getAction()){
			...
			case MotionEvent.ACTION_MOVE:
				if (mUserIsMovingPointer){
					mAngle = (float) java.lang.Math.atan2(	y,
															x);
					mPointerColor.setColor(calculateColor(mAngle));
	...
	protected Parcelable onSaveInstanceState(){
		...
		state.putParcelable(STATE_PARENT,
							superState);
		state.putFloat(	STATE_ANGLE,
						mAngle);
		state.putInt(	STATE_OLD_COLOR,
						mCenterOldColor);
