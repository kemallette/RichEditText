...
public class SVBar	extends
	...
	private void init(AttributeSet attrs, int defStyle){
		...
		mBarPointerPosition = (mBarLength / 2)
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh){
		...
		if (!isInEditMode())
			shader = new LinearGradient(mBarPointerHaloRadius,
										0,
										(mBarLength + mBarPointerHaloRadius),
										mBarThickness,
										new int[] {
	...
	protected void onDraw(Canvas canvas){
		...
		// Draw the bar.
		canvas.drawRect(mBarRect,
						mBarPaint);
		// Draw the pointer halo.
		canvas.drawCircle(	mBarPointerPosition,
							mBarPointerHaloRadius,
							mBarPointerHaloRadius,
		...
		if (x > (mBarPointerHaloRadius + (mBarLength / 2))
			...
			mColor = Color
							.HSVToColor(new float[] {
														mHSVColor[0],
												1f,
												1 - (mPosToSVFactor * (x - (mBarPointerHaloRadius + (mBarLength / 2)))) });
		else if (x > mBarPointerHaloRadius
					&& x < (mBarPointerHaloRadius + mBarLength))
			mColor = Color.HSVToColor(new float[] { mHSVColor[0],
								((mPosToSVFactor * (x - mBarPointerHaloRadius))),
								1f });
		else if (x == mBarPointerHaloRadius)
			mColor = Color.WHITE;
		else if (x == mBarPointerHaloRadius
	...
	protected Parcelable onSaveInstanceState(){
		...
		state.putParcelable(STATE_PARENT,
							superState);
		state.putFloatArray(STATE_COLOR,
							mHSVColor);
		float[] hsvColor = new float[3];
		Color.colorToHSV(	mColor,
							hsvColor);
		if (hsvColor[1] < hsvColor[2])
			state.putFloat(	STATE_SATURATION,
							hsvColor[1]);
		else
			state.putFloat(	STATE_VALUE,
							hsvColor[2]);
