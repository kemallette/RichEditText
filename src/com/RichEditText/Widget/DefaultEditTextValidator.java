...
import com.RichEditText.Validations.AlphaNumericValidator;
import com.RichEditText.Validations.AlphaValidator;
import com.RichEditText.Validations.AndValidator;
import com.RichEditText.Validations.CreditCardValidator;
import com.RichEditText.Validations.DomainValidator;
import com.RichEditText.Validations.DummyValidator;
import com.RichEditText.Validations.EmailValidator;
import com.RichEditText.Validations.EmptyValidator;
import com.RichEditText.Validations.IpAddressValidator;
import com.RichEditText.Validations.MultiValidator;
import com.RichEditText.Validations.NotValidator;
import com.RichEditText.Validations.NumericValidator;
import com.RichEditText.Validations.OrValidator;
import com.RichEditText.Validations.PhoneValidator;
import com.RichEditText.Validations.RegexpValidator;
import com.RichEditText.Validations.Validator;
import com.RichEditText.Validations.WebUrlValidator;
...
public class DefaultEditTextValidator implements EditTextValidator{
	...
	public DefaultEditTextValidator(EditText editText, AttributeSet attrs,
		...
		emptyAllowed =
		               typedArray.getBoolean(R.styleable.RichEditText_emptyAllowed,
		                                     false);
		testType = typedArray.getInt(R.styleable.RichEditText_testType,
		                             EditTextValidator.TEST_NOCHECK);
		testErrorString =
		                  typedArray.getString(R.styleable.RichEditText_testErrorString);
		classType = typedArray.getString(R.styleable.RichEditText_classType);
		customRegexp =
		               typedArray.getString(R.styleable.RichEditText_customRegexp);
		emptyErrorString =
		                   typedArray.getString(R.styleable.RichEditText_emptyErrorString);
		typedArray.recycle();
	...
	public void resetValidators(Context context){
		...
		defaultEmptyErrorString =
		                          context.getString(R.string.error_field_must_not_be_empty);
		...
		switch(testType){
			...
			case TEST_ALPHA:
				toAdd =
				        new AlphaValidator(TextUtils.isEmpty(testErrorString)
				                                                             ? context.getString(R.string.error_only_standard_letters_are_allowed)
				                                                             : testErrorString);
				break;
			case TEST_ALPHANUMERIC:
				toAdd =
				        new AlphaNumericValidator(TextUtils.isEmpty(testErrorString)
				                                                                    ? context.getString(R.string.error_this_field_cannot_contain_special_character)
				                                                                    : testErrorString);
				break;

			case TEST_NUMERIC:
				toAdd =
				        new NumericValidator(TextUtils.isEmpty(testErrorString)
				                                                               ? context.getString(R.string.error_only_numeric_digits_allowed)
				                                                               : testErrorString);
				break;
			case TEST_REGEXP:

				toAdd = new RegexpValidator(testErrorString,
				                            customRegexp);
				break;
			case TEST_CREDITCARD:
				toAdd =
				        new CreditCardValidator(TextUtils.isEmpty(testErrorString)
				                                                                  ? context.getString(R.string.error_creditcard_number_not_valid)
				                                                                  : testErrorString);
				break;
			case TEST_EMAIL:
				toAdd =
				        new EmailValidator(TextUtils.isEmpty(testErrorString)
				                                                             ? context.getString(R.string.error_email_address_not_valid)
				                                                             : testErrorString);
				break;
			case TEST_PHONE:
				toAdd =
				        new PhoneValidator(TextUtils.isEmpty(testErrorString)
				                                                             ? context.getString(R.string.error_phone_not_valid)
				                                                             : testErrorString);
				break;
			case TEST_DOMAINNAME:
				toAdd =
				        new DomainValidator(TextUtils.isEmpty(testErrorString)
				                                                              ? context.getString(R.string.error_domain_not_valid)
				                                                              : testErrorString);
				break;
			case TEST_IPADDRESS:
				toAdd =
				        new IpAddressValidator(TextUtils.isEmpty(testErrorString)
				                                                                 ? context.getString(R.string.error_ip_not_valid)
				                                                                 : testErrorString);
				break;
			case TEST_WEBURL:
				toAdd =
				        new WebUrlValidator(TextUtils.isEmpty(testErrorString)
			...
			case TEST_CUSTOM:
				...
				if (TextUtils.isEmpty(testErrorString)){
					throw new RuntimeException(String.format("Trying to create a custom validator (%s) but no error string specified.",
					                                         classType));
				}
				...
				try{
					...
					if (!Validator.class.isAssignableFrom(loadedClass)){
						throw new RuntimeException(String.format("Custom validator (%s) does not extend %s",
						                                         classType,
						                                         Validator.class.getName()));
					}
					customValidatorClass =
					                       (Class<? extends Validator>) loadedClass;
				}catch(ClassNotFoundException e){
					throw new RuntimeException(String.format("Unable to load class for custom validator (%s).",
					                                         classType));
				}
				...
				try{
					...
					throw new RuntimeException(String.format("Unable to construct custom validator (%s) with argument: %s",
					                                         classType,
					                                         testErrorString));
				}
		...
		if (!emptyAllowed){ // If the xml tells us that this is a required field, we will add the
			                // EmptyValidator.
			...
			tmpValidator =
			               new OrValidator(toAdd.getErrorMessage(),
			                               new NotValidator(null,
			                                                new EmptyValidator(null)),
			                               toAdd);
		}
